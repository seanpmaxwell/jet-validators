...
async function main() {
  ...
  console.log(`Benchmark results written to ${fileURLToPath(README_FILE)}.`);
}
...
async function writeResults(results: BenchmarkResult[]) {
  const readmePath = fileURLToPath(README_FILE);
  const originalReadme = await fs.readFile(readmePath, 'utf8');
  const section = buildBenchmarksSection(results);
  const updatedReadme = replaceBenchmarksSection(originalReadme, section);
  await fs.writeFile(readmePath, updatedReadme, 'utf8');
}

function buildBenchmarksSection(results: BenchmarkResult[]): string {
  const rows = results
    .map(
      (result) =>
...
function replaceBenchmarksSection(readme: string, section: string): string {
  const headerRegex = /## Benchmarks[^\n]*\n/;
  const match = readme.match(headerRegex);
  if (!match) {
    throw new Error('Benchmarks section not found in README.md');
  }
  const startIdx = match.index ?? 0;
  const nextSectionIdx = readme.indexOf('\n## ', startIdx + match[0].length);
  const endIdx = nextSectionIdx === -1 ? readme.length : nextSectionIdx;
